...
Ultimate Sovereign Dark-Mode 6-Slide Smart India Hackathon (SIH 2026) Winning Presentation Deck.
Military-grade Deep Navy & Tech Cyan industrial aesthetic across all 6 slides.
Adheres strictly to the official SIH 6-slide template:
1. Title Page
...
# Sovereign Military-Grade Dark Palette
COLOR_BG_DARK       = RGBColor(7, 15, 30)        # #070F1E Deep Void Navy
COLOR_NAVY_SURFACE  = RGBColor(15, 27, 48)       # #0F1B30 Surface Navy
COLOR_NAVY_CARD     = RGBColor(23, 37, 65)       # #172541 Elevated Dark Card
COLOR_CYAN_ACCENT   = RGBColor(56, 189, 248)     # #38BDF8 Glowing Tech Cyan
COLOR_EMERALD       = RGBColor(16, 185, 129)     # #10B981 Verified Emerald
COLOR_CRIMSON       = RGBColor(239, 68, 68)      # #EF4444 Alert Crimson
COLOR_AMBER         = RGBColor(245, 158, 11)     # #F59E0B Warning/Feature Amber
COLOR_VIOLET        = RGBColor(168, 85, 247)     # #A855F7 AI Neural Violet
COLOR_WHITE         = RGBColor(255, 255, 255)    # Pure Crisp White
COLOR_TEXT_PRIMARY  = RGBColor(241, 245, 249)    # #F1F5F9 Heading Slate
COLOR_TEXT_MUTED    = RGBColor(148, 163, 184)    # #94A3B8 Body Muted Silver
COLOR_BORDER_CYAN   = RGBColor(14, 116, 144)     # #0E7490 Border Muted Cyan
COLOR_BORDER_CARD   = RGBColor(30, 58, 102)      # #1E3A66 Crisp Card Border
COLOR_TABLE_ALT     = RGBColor(12, 22, 40)       # #0C1628 Dark Table Row
...
def build_deck(output_path: str):
    ...
    def add_header(slide, title_text: str, subtitle_text: str, badge_text: str = "SIH 2026 — PS 26117"):
        header_box = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(0.35), Inches(11.733), Inches(0.9))
        header_box.fill.solid()
        header_box.fill.fore_color.rgb = COLOR_NAVY_SURFACE
        header_box.line.color.rgb = COLOR_CYAN_ACCENT
        header_box.line.width = Pt(1.5)

        tb = slide.shapes.add_textbox(Inches(1.0), Inches(0.42), Inches(8.5), Inches(0.78))
        tf = tb.text_frame
        tf.word_wrap = True
        ...
        bb = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(9.8), Inches(0.52), Inches(2.5), Inches(0.52))
        bb.fill.solid()
        bb.fill.fore_color.rgb = COLOR_NAVY_CARD
        bb.line.color.rgb = COLOR_AMBER
        bb.line.width = Pt(1.25)
        btf = bb.text_frame
        bp = btf.paragraphs[0]
        bp.text = badge_text
        bp.font.bold = True
        bp.font.size = Pt(9)
        bp.font.color.rgb = COLOR_AMBER
    ...
    def add_footer(slide, slide_num: int):
        ...
        tf.margin_left = tf.margin_top = tf.margin_right = tf.margin_bottom = 0
        p = tf.paragraphs[0]
        p.text = f"INDRA SOVEREIGN WORKBENCH • AIR-GAP ZERO-WAN AUDITED • SM-26117 • SLIDE {slide_num} OF 6"
        p.font.size = Pt(8)
        p.font.color.rgb = COLOR_TEXT_MUTED
        p.font.name = "Calibri"

    def add_dark_card(slide, x, y, w, h, title, val, unit, desc, border_col=COLOR_CYAN_ACCENT, bg_col=COLOR_NAVY_CARD):
    ...
    # =========================================================================
    # SLIDE 1: TITLE PAGE (MILITARY DARK HERO)
    # =========================================================================
    s1 = prs.slides.add_slide(blank_layout)
    set_slide_background(s1)

    # Top Tag
    tb1_top = s1.shapes.add_textbox(Inches(1.0), Inches(0.7), Inches(11.333), Inches(0.4))
    ...
    p_sih.font.color.rgb = COLOR_CYAN_ACCENT

    # Main Hero Title
    tb1_hero = s1.shapes.add_textbox(Inches(1.0), Inches(1.2), Inches(11.333), Inches(1.8))
    ...
    # 3 Hero Pillars
    cards_data = [
        ("Problem Statement ID", "26117", "Theme: Smart Automation • Category: Software", COLOR_CYAN_ACCENT),
    ...
    for idx, (title, val, desc, col) in enumerate(cards_data):
        cx = 1.0 + idx * 3.9
        add_dark_card(s1, cx, 3.25, 3.6, 1.55, title, val, "", desc, col)

    # Architectural Overview Banner
    team_card = s1.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(1.0), Inches(5.1), Inches(11.333), Inches(1.5))
    team_card.fill.solid()
    team_card.fill.fore_color.rgb = COLOR_NAVY_SURFACE
    team_card.line.color.rgb = COLOR_CYAN_ACCENT
    team_card.line.width = Pt(1.5)

    ttb = s1.shapes.add_textbox(Inches(1.2), Inches(5.2), Inches(10.9), Inches(1.25))
    ttf = ttb.text_frame
    ttf.word_wrap = True
    tp1 = ttf.paragraphs[0]
    tp1.text = "CORE SOVEREIGN SPECIFICATIONS & VERIFIED REPOSITORY"
    tp1.font.bold = True
    tp1.font.size = Pt(10)
    tp1.font.color.rgb = COLOR_CYAN_ACCENT

    tp2 = ttf.add_paragraph()
    tp2.text = "• Resident Open-Weight LLMs: Qwen 2.5 Coder (7B) + Qwen 3 (8B) loaded in GPU memory with zero cloud egress.\n" \
               "• Deterministic Engineering Solvers: ASME B31.3 §304.1.2 hoop stress, Crane TP 410 hydraulics, ISO 10816-3 vibration triage.\n" \
               "• Live Repository: https://github.com/prushidhar/SIH260117  |  Demo Video: https://youtu.be/WQ_M94EY9do"
    tp2.font.size = Pt(9)
    tp2.font.color.rgb = COLOR_TEXT_PRIMARY

    # =========================================================================
    # SLIDE 2: INNOVATION AND UNIQUENESS (DARK COMPARISON TABLE)
    # =========================================================================
    s2 = prs.slides.add_slide(blank_layout)
    set_slide_background(s2)
    add_header(s2, "2. Innovation & Uniqueness", "Architectural Moat vs Cloud LLMs & Generic Local Wrappers")
    add_footer(s2, 2)

    # Left: Comparison Matrix Table
    tbl_shape = s2.shapes.add_table(6, 4, Inches(0.8), Inches(1.45), Inches(7.6), Inches(5.2))
    ...
    for c_idx, h in enumerate(headers):
        ...
        cell.fill.solid()
        cell.fill.fore_color.rgb = COLOR_NAVY_CARD
        cell.text = h
    ...
    for r_idx, row in enumerate(rows_data):
        for c_idx, val in enumerate(row):
            ...
            cell.fill.solid()
            cell.fill.fore_color.rgb = COLOR_NAVY_SURFACE if r_idx % 2 == 0 else COLOR_TABLE_ALT
            cell.text = val
            p = cell.text_frame.paragraphs[0]
            p.font.size = Pt(8.5)
            p.font.color.rgb = COLOR_TEXT_MUTED
    ...
    # Right: 3 Dark Innovation Highlight Cards
    right_cards = [
        ...
        ("Automated Deliverable Trinity", ".DOCX + .XLSX + .PPTX",
         "Auto-compiles formal Word statutory notes, Excel engineering calculation sheets, and 16:9 executive presentation decks in <3.5 seconds.", COLOR_CYAN_ACCENT),
    ...
    for idx, (title, val, desc, col) in enumerate(right_cards):
        add_dark_card(s2, 8.6, 1.45 + idx * 1.75, 3.9, 1.6, title, val, "", desc, col)

    # =========================================================================
    # SLIDE 3: TECHNICAL APPROACH (DARK MULTI-AGENT STATE MACHINE)
    # =========================================================================
    ...
    sh_card.fill.solid()
    sh_card.fill.fore_color.rgb = COLOR_NAVY_SURFACE
    ...
    # =========================================================================
    # SLIDE 4: FEASIBILITY AND VIABILITY (DARK RISK & HARDWARE SIZING)
    # =========================================================================
    ...
    risks = [
        ...
        ("RISK 2: Confidential Data Leakage", "Strict 0-WAN Loopback Architecture",
         "No cloud endpoints. Live network daemon actively monitors socket bindings. Any unauthorized egress terminates the connection immediately. Confirmed by SHA-256 proof.", COLOR_CYAN_ACCENT),
    ...
    for idx, (title, val, desc, col) in enumerate(risks):
        add_dark_card(s4, 0.8, 1.45 + idx * 1.75, 6.0, 1.6, title, val, "", desc, col)

    # Right: Hardware Viability & Operational Proof
    add_dark_card(s4, 7.1, 1.45, 5.4, 1.6, "Hardware Sizing", "Single Workstation", "",
                  "Runs locally on 1x RTX 4080 (16GB VRAM) or standard 32GB RAM CPU workstations. No high-cost enterprise cloud servers required.", COLOR_CYAN_ACCENT)

    add_dark_card(s4, 7.1, 3.2, 5.4, 1.6, "Execution Latency", "< 3.5 Seconds", "",
    ...
    add_dark_card(s4, 7.1, 4.95, 5.4, 1.6, "Operating Economics", "₹0 Recurring Cost", "",
                  "Zero per-token cloud API fees. Saves ₹15–30 Lakhs annually per plant unit while guaranteeing 100% data sovereignty.", COLOR_AMBER)

    # =========================================================================
    # SLIDE 5: IMPACT AND BENEFITS (DARK QUANTITATIVE METRICS)
    # =========================================================================
    ...
    kpis = [
        ...
        ("Cloud API Costs", "₹0", "SAVED", "Zero external tokens; eliminates recurring cloud bills", COLOR_CYAN_ACCENT),
        ("Air-Gap Compliance", "100%", "ZERO-WAN", "Meets Indian Ministry of Petroleum cybersecurity standards", COLOR_VIOLET),
        ("Math Hallucination", "0.0%", "DETERMINISTIC", "AST verified calculations eliminate catastrophic pipe rupture risk", COLOR_EMERALD),
    ]
    for idx, (title, val, unit, desc, col) in enumerate(kpis):
        add_dark_card(s5, 0.8 + idx * 2.95, 1.45, 2.8, 1.6, title, val, unit, desc, col)

    # Bottom 4 Stakeholder Value Cards
    stakeholders = [
        ("Refinery Asset Integrity Engineers",
         "• Instant Ultrasonic NDT wall thickness & RSL evaluations\n• ASME B31.3 §304.1.2 compliance matrices auto-generated\n• Replaces error-prone manual engineering spreadsheets", COLOR_CYAN_ACCENT),
    ...
    # =========================================================================
    # SLIDE 6: RESEARCH, REFERENCES & ROADMAP (DARK STANDARDS & TRL 7-9)
    # =========================================================================
    ...
    left_card.fill.fore_color.rgb = COLOR_NAVY_CARD
    left_card.line.color.rgb = COLOR_CYAN_ACCENT
    ...
    lp1.font.bold = True
    lp1.font.size = Pt(10)
    lp1.font.color.rgb = COLOR_CYAN_ACCENT
    ...
    lp2.font.color.rgb = COLOR_TEXT_MUTED
    ...
    print(f"[+] Successfully generated Sovereign Dark-Mode SIH Winning Deck: {output_path} ({os.path.getsize(output_path)} bytes)")
    return output_path
